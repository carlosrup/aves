...
proc read*(
  _: typedesc[Exif], io: KaitaiStream, root: KaitaiStruct, parent: KaitaiStruct
): Exif

proc read*(
  _: typedesc[Exif_ExifBody], io: KaitaiStream, root: KaitaiStruct, parent: Exif
): Exif_ExifBody

proc read*(
  _: typedesc[Exif_ExifBody_Ifd],
  io: KaitaiStream,
  root: KaitaiStruct,
  parent: KaitaiStruct,
): Exif_ExifBody_Ifd
...
proc read*(
  _: typedesc[Exif_ExifBody_IfdField],
  io: KaitaiStream,
  root: KaitaiStruct,
  parent: Exif_ExifBody_Ifd,
): Exif_ExifBody_IfdField
...
proc read*(
  ...
  this.root = root
  this.parent = parent
  this.isLe = cast[Exif_ExifBody_Ifd](this.parent).isLe
...
proc typeByteLength(this: Exif_ExifBody_IfdField): int8 =
  ...
  let typeByteLengthInstExpr = int8(
    (
      if this.fieldType == exif.word: 2
      else: (if this.fieldType == exif.dword: 4 else: 1)
    )
  )
...
proc byteLength(this: Exif_ExifBody_IfdField): int =
  ...
  let byteLengthInstExpr = this.length.int * this.typeByteLength.int
...
proc fromFile*(
    _: typedesc[Exif_ExifBody_IfdField], filename: string
): Exif_ExifBody_IfdField =
